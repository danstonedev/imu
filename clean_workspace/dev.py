...
def check_environment():
    ...
    try:
        ...
        print(
            f"✓ Environment OK: FastAPI {fastapi.__version__}, Uvicorn {uvicorn.__version__}"
        )
        return True
...
def main():
    ...
    try:
        with socket.create_connection((host, port), timeout=0.5):
            print(
                f"⚠ Port {port} is already in use on {host}. If an old server is stuck, run scripts/stop-port.ps1 -Port {port}."
            )
    except OSError:
        pass

    print(
        f"Starting development server on {host}:{port} (reload={'on' if reload else 'off'}) in {project_dir}"
    )

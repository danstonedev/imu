...
def metrics(
    ...
    rms = (
        np.sqrt(np.mean(means**2, axis=0)) if means.size else np.array([np.nan, np.nan])
    )
...
def main():
    ...
    ap.add_argument(
        ...
        type=str,
        required=False,
        default=str(Path(__file__).resolve().parents[1] / "sample data"),
        help="Path to folder with demo 5 CSVs",
    )
    ...
    for m in modes:
        Ac = apply_baseline_correction(
            t, A, strides, cfg, yaw_pelvis=yaw_p, yaw_femur=yaw_f, mode=m
        )
        ...
        results[m] = {
            "angles_deg": Ac_deg,
            "dc_shift_deg": dc,
            "drift_slope_deg_per_min": slope,
            "per_stride_mean_rms_deg": rms,
        }
    ...
    if strides:
        ...
        print(
            "stride_len(samples): min/med/max=",
            int(np.min(lens)),
            int(np.median(lens)),
            int(np.max(lens)),
        )
    ...
    for m in modes:
        ...
        rows.append(
            [
                m,
                f"dc(Y,Z)={dc[0]:.2f},{dc[1]:.2f}",
                f"slope(Y,Z)={slope[0]:.2f},{slope[1]:.2f}",
                f"strideRMS(Y,Z)={rms[0]:.2f},{rms[1]:.2f}",
            ]
        )
    print(
        pd.DataFrame(
            rows,
            columns=["mode", "dc_shift", "drift_slope_per_min", "per_stride_mean_rms"],

...
def summarize_angles(
    A: np.ndarray, t: np.ndarray, stance: np.ndarray, start_margin=2.0, end_margin=2.0
):
    ...
    if b <= a:
        ...
        T = (t[b - 1] - t[a]) / 60.0
        slope = (
            np.nanmean(A[b - 50 : b], axis=0) - np.nanmean(A[a : a + 50], axis=0)
        ) / max(T, 1e-6)
    ...
    return {
        ...
        "abs_mean_diff": (
            (np.abs(m1 - m0)).tolist() if isinstance(m0, np.ndarray) else [np.nan] * 3
        ),
        "drift_slope_deg_per_min": (
            slope.tolist() if isinstance(slope, np.ndarray) else [np.nan] * 3
        ),
        "max_abs_first10": mx0,
        "max_abs_last10": mx1,
...
def run_matrix():
    ...
    optsA = dict(
        base_opts, angles_baseline_mode="yaw_share_only", angles_highpass_fc=None
    )
    # B: stride-debias only
    optsB = dict(
        base_opts, angles_baseline_mode="stride_debias_only", angles_highpass_fc=None
    )

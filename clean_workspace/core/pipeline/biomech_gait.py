...
def detect_hs_to_biomech(ax: np.ndarray, az: np.ndarray, gx: np.ndarray, fs: float,
    ...
    Args:
        ax: Anterior-posterior acceleration (m/s²)
        az: Vertical acceleration (m/s²), gravity removed
        gx: Gyro pitch (rad/s) about mediolateral axis
        fs: Sampling rate (Hz)
        vib_band: Frequency band for impact vibrations
        gx_lp: Lowpass cutoff for gyro smoothing
        hs_jerk_k: Multiplier for adaptive HS threshold
        to_win_ms: Search window for TO in ms
        
    Returns:
        HS indices, TO indices
    """
    N = len(az)
    
    # Prevent filter issues with short data
    if N < 100:
        return np.array([], dtype=int), np.array([], dtype=int)
    
    # Filtered signals
    vib = sosfiltfilt(_sos_bp(*vib_band, fs), az)               # vertical vibration
    gx_f = sosfiltfilt(_sos_lp(gx_lp, fs), gx)                  # smooth pitch rate
    dgx = np.gradient(gx_f) * fs                                # pitch angular acceleration
    jerk = np.gradient(vib) * fs
    
    # --- HS detection (impact + gyro confirmation) ---
    # Adaptive threshold using robust scale (MAD≈1.4826*median|x-median|)
    mad = 1.4826 * np.median(np.abs(vib - np.median(vib)))
    if mad < 1e-6:  # Prevent division by near-zero
        mad = np.std(vib)
    
    hs_peaks, _ = find_peaks(np.abs(vib), height=hs_jerk_k*mad, distance=int(0.35*fs))
    HS = []
    for p in hs_peaks:
        # gyro confirmation: near a zero-cross or sharp slope change
        w0, w1 = max(0, p-int(0.04*fs)), min(N-1, p+int(0.04*fs))
        if w1 <= w0:
            continue
            
        # Check for sign change (zero crossing) or sharp slope
    ...
    # --- TO detection after each HS ---
    TO = []
    min_gap = int(0.25*fs)  # Minimum stance duration
    
    for i, hs in enumerate(HS):
        # search window: [hs + 25%GC … hs + 80%GC]
        start = hs + int(0.15*fs)  # Start search earlier
        stop = hs + int(0.8*fs)
        if i+1 < len(HS):
        ...
        # Candidate = local minimum of gx_f then rising slope + drop in vib RMS
        w = max(1, int(0.06*fs))  # ~60 ms window, at least 1 sample
        
        # rolling vibration RMS (simple convolution)
        if w < len(vib):
            vib_rms = np.sqrt(np.convolve(vib**2, np.ones(w)/w, mode='same'))
        else:
            vib_rms = np.abs(vib)
        
        # look for minima in gx_f within search window
        ...
        # Also consider simple minimum in search window if no peaks found
...
def bilateral_gait_analysis(t_left: np.ndarray, a_left: np.ndarray, g_left: np.ndarray,
                           ...
                           fs: float) -> dict:
    """
    Perform bilateral gait analysis using biomechanical event detection.
    
    Args:
        t_left, t_right: Time arrays
        a_left, a_right: Acceleration arrays (Nx3, assume [AP, ML, Vert])
        g_left, g_right: Gyroscope arrays (Nx3, assume [pitch, roll, yaw])
        fs: Sampling rate
        
    Returns:
        Dictionary with gait events and analysis
    """
    # Extract relevant signals (assuming standard IMU orientation)
    # AP = anterior-posterior (X), Vertical = Z, Pitch = X (sagittal plane rotation)
    ax_l, az_l = a_left[:, 0], a_left[:, 2]  
    ax_r, az_r = a_right[:, 0], a_right[:, 2]
    gx_l, gx_r = g_left[:, 0], g_right[:, 0]  # Pitch gyro
    
    # Remove gravity from vertical acceleration (simple high-pass)
    # For walking, gravity is ~9.8 m/s^2 downward
    az_l_filt = az_l - np.mean(az_l)  # Simple detrend
    az_r_filt = az_r - np.mean(az_r)
    
    # Detect events for each leg
    ...
    # Analyze bilateral coordination
    all_events = []
    if len(hs_l_time) > 0:
    ...
    # Calculate step and stride times if we have enough events
    if len(hs_l_time) > 1:
...
def detect_heel_strikes_biomech(t: np.ndarray, omega_W: np.ndarray, a_free_W: np.ndarray, 
    ...
    """
    Detect heel strikes using biomechanical signatures:
    1. Sharp vertical acceleration spike (impact)
    2. Sagittal plane gyro zero-crossing/extremum (rotation change)
    3. High jerk in impact band (15-40 Hz)
    """
    
    # 1. Preprocess signals
    # High-pass filter acceleration to remove gravity/low freq drift
    sos_hp = signal.butter(2, 0.5, btype='high', fs=fs, output='sos')
    a_hp = signal.sosfiltfilt(sos_hp, a_free_W, axis=0)
    
    # Band-pass acceleration for impact detection (10-35 Hz)
    sos_impact = signal.butter(2, [10, 35], btype='band', fs=fs, output='sos')
    a_impact = signal.sosfiltfilt(sos_impact, a_free_W, axis=0)
    
    # Low-pass gyro for smooth rotation tracking (10 Hz)
    sos_lp = signal.butter(2, 10, btype='low', fs=fs, output='sos')
    omega_smooth = signal.sosfiltfilt(sos_lp, omega_W, axis=0)
    
    # 2. Find acceleration impact candidates
    # Vertical component is typically Z (assuming standard IMU orientation)
    a_vert = a_impact[:, 2]  # Z-axis
    ...
    # Adaptive threshold for jerk detection
    jerk_std = np.std(jerk_resultant)
    jerk_threshold = 3.0 * jerk_std  # 3-sigma threshold
    
    # Find local maxima in acceleration with high jerk
    from scipy.signal import find_peaks
    
    # Find peaks in vertical acceleration
    min_height = np.mean(np.abs(a_vert)) + 2 * np.std(np.abs(a_vert))
    min_distance = int(0.4 * fs)  # Minimum 0.4s between heel strikes
    ...
    # Filter by jerk criterion
    high_jerk_peaks = []
    for peak in accel_peaks:
        if np.abs(jerk_resultant[peak]) > jerk_threshold:
            high_jerk_peaks.append(peak)
    
    # 3. Confirm with gyroscope zero-crossings/extrema
    # Sagittal plane rotation (typically Y-axis for pitch)
    omega_sagittal = omega_smooth[:, 1]  # Y-axis pitch rotation
    
    # Find zero crossings and local extrema
    zero_crossings = []
    for i in range(1, len(omega_sagittal)):
        if omega_sagittal[i-1] * omega_sagittal[i] < 0:  # Sign change
    ...
    # 4. Match acceleration peaks with nearby gyro events
    confirmed_heel_strikes = []
    tolerance_samples = int(0.05 * fs)  # ±50ms tolerance
    
    for accel_peak in high_jerk_peaks:
        # Find closest gyro event
        closest_gyro = None
...
def bilateral_gait_cycles(t_L: np.ndarray, hs_L: np.ndarray, 
    ...
    """
    Create proper bilateral gait cycles with alternating pattern
    """
    # Convert heel strikes to common time base (use left leg timing)
    if len(hs_L) == 0 or len(hs_R) == 0:
        return {"L": hs_L, "R": hs_R, "alternating_score": 0.0}
    
    # Interpolate right heel strikes to left time base
    hs_R_time = t_R[hs_R]
    hs_R_interp = np.interp(hs_R_time, t_L, np.arange(len(t_L)))
    hs_R_interp = hs_R_interp.astype(int)
    
    # Combine all heel strikes with leg labels
    all_events = []
    for hs in hs_L:
...
def cycle_mean_sd_biomech(t: np.ndarray, sig: np.ndarray, heel_strikes: np.ndarray, 
    ...
    """
    Calculate cycle mean and SD using biomechanically-detected heel strikes
    """
